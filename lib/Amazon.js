...
class AmazonScraper {
    ...
    async startScraper() {
        ...
        return jsonData; // Return the JSON string
    }
    ...
    async extractCategories() {
        ...
        const $ = cheerio.load(body.replace(/\s\s+/g, '').replace(/\n/g, ''));
        const categorySelect = $('#searchDropdownBox').get(0); // Use get(0) to access the first element
        
        if (!categorySelect || !Array.isArray(categorySelect.children)) {
            throw new Error("Can't find category selector or it has no children");
        }
